...
from ..schemas.loan import (
    LoanCreate, LoanOut, TrancheCreate, TrancheOut,
...
def create_loan(data: LoanCreate, db: Session = Depends(get_db)):
    ...
    loan = Loan(
        ...
        collateral_description_hi=data.collateral_description_hi,
        interest_rate=data.interest_rate or 3.0,
        notes=data.notes,
    )
    db.add(loan)
    db.flush()  # get loan.id

    tranche_date = data.first_tranche_date or datetime.now(timezone.utc)
    tranche = LoanTranche(
        loan_id=loan.id,
        amount=data.first_tranche_amount,
...
def add_payment(loan_id: int, data: PaymentCreate, db: Session = Depends(get_db)):
    ...
    # ── Auto-close: if outstanding hits zero, close the loan automatically ──
    db.refresh(loan)
    summary = calc_loan_summary(loan.tranches, loan.payments, loan.interest_rate)
